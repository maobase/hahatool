...
### 视觉升级：专题封面换成免版权高质量照片（目标 #1 #3，按用户「不够高级」反馈）
专题卡片原来是扁平 SVG 渐变封面，显得廉价。改用 **Unsplash 免版权精选照片**，每个专题配一张主题相关图（编程=蓝色代码、学习=图书馆、数字人=播客麦克风、视频=电影摄影机、办公=现代工位、搜索=数据科技…）：
- 9 张照片人工挑选后**下载入自有对象存储**（`topics/photo/*.jpg`，遵守目标 #3 不外链第三方），更新各专题 `topic_cover` meta。
- 卡片封面加 **品牌色渐变遮罩**（`.topic-card-cover::after`，用 `--brand-900` 随 4 套主题色自适应），把 9 张不同照片统一成同一套编辑式视觉语言——既真实高级又不杂乱。
- 取图前先下了一版 LoremFlickr 候选对比，质量偏随机（木凳/路人）果断弃用，改走人工挑选的 Unsplash。
- 影响 `/topics` 与首页「精选专题」，浅/深两色均验证；脚本 `scripts/seed-topic-photos.sh` 记录选图与流程、幂等可重跑。主题 1.6.121 → 1.6.122。

## [v1.6.119] - 2026-06-24

### 视觉去「模板感」第一步：首页 Hero 重做（目标 #1，按用户「整体太 generic」反馈）
用户反馈整站像通用 AI 模板。先从第一印象的 Hero 开刀，去掉两个最典型的「AI SaaS 套路」元素、建立可复用的签名视觉：
- **删除 eyebrow 胶囊**（「✨ 全球 AI 工具中文导航」那种小标签——impeccable 准则里的 AI 语法标志）。
- **删除盒装三指标面板**（28+/9+/35+ 带分隔线的 metric box——hero-metric 模板套路），改为一行**克制的 inline 元信息**：`● 每日更新 · 已收录 28+ 款工具 · 9 大分类 · 35+ 资讯`，含一个品牌色**呼吸式 live 圆点**（带 reduced-motion 关闭）。
- **签名记号**：标题「AI 工具」加品牌色 marker 下划线（`.hl::after`），作为可向全站延展的识别元素；标题更自信（clamp 56→60、字距 -0.025em）。
- Hero 更短更聚焦，浅/深、桌面/移动四态截图确认。主题 1.6.111 → 1.6.112。
- 这是去 generic 的方向样板；后续把同一套语言（克制信息密度、签名记号、编辑式层级）延展到版块标题、卡片与详情页。

## [v1.6.118] - 2026-06-24

### 打磨（迭代 75：用 /impeccable 规范做全站可读性与质感基线 —— 目标 #1）
按 impeccable 设计准则审计现有设计系统，修正最影响「廉价感/不够精致」的全站问题：
- **次级文字对比**：`--text-3` 浅色 `#9ca3af`（约 2.5:1，低于 WCAG AA）→ `#6b7280`；深色 `#6b7280` → `#8b95a4`。该色用于所有 `.muted`、版块副标题、卡片底部、日期、计数、热榜元信息——一处改动，全站日期/元信息从「灰到看不清」变清晰。
- **占位符可读**：显式 `::placeholder{color:var(--text-3)}`，搜索框提示文字不再过浅。
- **品牌化选区**：`::selection` 用品牌色（浅/深各一套），细节更精致。
- **标题排版**：`h1,h2,h3` 加 `text-wrap:balance`，多行标题断行更均匀。
- **动效无障碍**：`prefers-reduced-motion` 下关闭平滑滚动。
- 审计确认既有体系达标项（无渐变文字、无超圆角、卡片 hover 抬升 + 边框高亮、全局 `:focus-visible` 焦点环、动效含 reduced-motion），未改动。主题 1.6.110 → 1.6.111。

## [v1.6.117] - 2026-06-22

### 修复 / 打磨（迭代 74：按用户反馈修正首页分类条对齐 + 缩小 banner —— 目标 #1）
用户截图指出两处：
1. **分类粘性条未对齐**：`.catbar-inner` 缺 `max-width`/`margin:auto`，内容（精选 / 增长最快…）顶到视口最左，与居中的页面内容（ticker、卡片）错位。补 `max-width:var(--maxw);margin:0 auto`（与 `.wrap`、`.ticker-inner` 一致），「精选」现与 logo/内容列左缘对齐；移动端补 16px 内边距。
2. **首页推广 banner 过大**：压缩尺寸——内边距 28px → 18/20px、Logo 56 → 46、标题 20 → 17px、简介 2 行 → 1 行、按钮加紧凑内边距、各间距收紧，整卡高度约降三成。
- 主题 1.6.108 → 1.6.109。实测：1440/390 两宽度截图确认分类条对齐、banner 更紧凑，浅色正常。

## [v1.6.116] - 2026-06-22

### 体验（迭代 73：全网热榜「回看自动刷新」+ 多项系统核查 —— 目标 #5）
- **/hot 自动刷新**：热榜数据每 5 分钟刷新一次；新增轻量脚本——用户切走再回到热榜页且已超 5 分钟时自动 `reload` 拉取最新（用 `visibilitychange` 判可见，不打断正在阅读的人，无轮询开销）。
- **核查并确认**（无需改动）：① 5 分钟缓存确实生效（Redis DB7 主键 `hahatool_hot` TTL 实测 274s/300s，stale 兜底 ~1 天）；② momoyu `type=1` 只是同 13 源的分组视图、其余 type 与 type=0 同源，故不做分类 Tab（无新数据）；③ 资讯/工具的站内浏览量 `views` 已对两类都自增（`functions.php` wp_footer + theme.js），「热门资讯」排序真实有效。
- 本轮搜罗资讯逐条核实后均放弃：36氪「融资决战周」为 5/11 旧闻、GLM-5.2/「Claude 5 关停」与一手来源冲突、其余为前瞻或聚合摘要——坚持不编造不上旧闻。
- 主题 1.6.107 → 1.6.108。

## [v1.6.115] - 2026-06-22

### 打磨（迭代 72：首页热榜 teaser 精选科技源，更贴合站点调性 —— 目标 #1 #5）
上轮热榜放开为全量源后，首页 teaser 默认取前 3 源即知乎/豆瓣/微博（偏社会八卦），放在「AI 工具与资讯导航」首页略显跳脱。
- 首页 teaser 改为**优先精选 3 个科技源**（IT之家/虎嗅/爱范儿/掘金/CSDN/中关村等，无则回退前 3），实测显示「IT之家 / 中关村在线 / 爱范儿」；teaser 副标题同步以科技源领衔。
- **`/hot` 全网热榜页仍为完整 13 源**（含知乎/微博/B站），点「完整热榜」即达——首页精选不削减完整页。
- 顺带补上轮欠的暗色验收：重排后的「专题 → 资讯 → 全网热榜」带在浅/深两色均正常。
- 主题 1.6.106 → 1.6.107。

## [v1.6.114] - 2026-06-22

### 按用户反馈：首页前置资讯/专题/热榜 + 热榜全量源 + 5 分钟刷新（目标 #2 #5）
用户反馈三点，本轮一并处理：
1. **热榜「比 momoyu 少」**：原因是迭代 60 我把 momoyu 的 13 源过滤成 6 个科技源。现取消过滤，`hahatool_fetch_hot()` 收录全部源（与 momoyu `?type=0` 一致，实测 13 源 / 154 条），并把请求改用 `?type=0`。页面相应从「AI · 科技热榜」更名为 **「全网热榜」**（H1 / `<title>` / 面包屑 / JSON-LD / meta 描述 / 首页 teaser 同步）。
2. **5 分钟更新**：热榜服务端缓存 15 分钟 → **5 分钟**（仍保留一天兜底防 momoyu 偶发失败空页）；数据条「缓存刷新 15min」→「更新频率 5min」。
3. **首页资讯/专题/热榜更突出**：原本资讯与热榜在首页最底部（被几十张工具卡淹没）。把 **AI 资讯** 与 **全网热榜** 两个模块上移到「精选专题」之后、「最新收录」之前，形成「专题 → 资讯 → 热榜」内容前置带（JustNews 式）。
- 部署后删除旧 `hahatool_hot` transient 使全量源即时生效。主题 1.6.105 → 1.6.106。
- 实测：`/hot` 标题「全网热榜」、13 源、5min、`/wp-json/hahatool/v1/hot` 返回 13 源；首页 section 顺序 专题→资讯→全网热榜→最新收录；截图确认资讯（头条大图卡）与热榜（多源榜单）已前置显示。
