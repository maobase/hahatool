...
### 视觉升级：全部 35 篇资讯换真实配图（按用户「资讯里还不是真图」反馈，目标 #1 #2 #3）
资讯封面原为扁平 SVG，改用 **Unsplash 免版权真实照片**，逐篇按主题人工映射（AI/大模型→神经网络抽象、机器人→人形机器人、芯片→半导体特写、融资→金融行情、AI 视频→影视拍摄、编程/Agent→代码屏幕、智能眼镜→AR 眼镜、政策/大会→会场，等）：
- 8 个主题桶共挑 46 张候选，**35 篇各配一张不重复的相关照片**；全部下载入自有对象存储 `news/photo/*.jpg`（守目标 #3），重写各篇 `cover` meta。
- 影响 `/news` 列表（头条大图 + 缩略图）、首页「AI 资讯」、文章详情头图、社媒 og:image（实测 og:image 已指向照片、200）。
- 脚本 `scripts/seed-news-photos.sh` 记录逐篇选图映射、幂等可重跑。主题 1.6.123 → 1.6.124。

## [v1.6.123] - 2026-06-24

### 修复 + 内容（迭代 80：首页分类条间距 + 推广位换真实广告主档位 —— 按用户反馈）
- **分类粘性条与推广 banner「黏在一起」**：`<main>` 缺顶部留白，首个版块顶到 catbar。加 `padding-top:36px`，分类条与推广位之间有了呼吸感。
- **推广位换真实广告主档位**：AI 导航站真实付费广告主多是「API 中转聚合」「套壳镜像站」，用 ChatGPT/Midjourney 占位不真实。新增两个示例广告主——**雾町 API**（全系模型 API 中转聚合，低至 1.5 折）、**智核 AI 助手**（免梯直连 GPT/Claude/绘画 镜像站），各配干净品牌图标（闪电/星芒渐变 SVG，入对象存储）；取消 ChatGPT/Midjourney 的 banner 标记（仍作普通工具）。链接指向各自站内详情页避免死链，详情标注「示例推广位」保持诚实。`scripts/seed-promo-advertisers.php` 幂等。主题 1.6.122 → 1.6.123。
