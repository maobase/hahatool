...
export default function FavoriteButton({
      ...
      <button
        ...
        className={`inline-flex min-h-11 items-center justify-center gap-1.5 rounded-xl border px-4 text-sm font-medium transition ${
          fav
            ...
            : 'border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 text-gray-600 dark:text-gray-400 hover:border-rose-200 hover:text-rose-500'
        }`}
      >
  ...
  return (
    <button
      ...
      aria-pressed={fav}
      aria-label={fav ? '取消收藏' : '收藏'}
      className="relative z-10 flex min-h-9 min-w-9 items-center justify-center rounded-lg text-gray-300 transition hover:bg-rose-50 dark:hover:bg-rose-900/30 hover:text-rose-500"
    >
      <Heart size={16} className={fav ? 'fill-rose-500 text-rose-500' : ''} />
    </button>
  );
