...
export default function FaqList({ faq, name }: { faq: { q: string; a: string }[]; name: string }) {
  ...
  return (
    <section className="mt-8 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm sm:p-8" aria-label="常见问题">
      ...
      <div className="mt-4 divide-y divide-gray-100">
        {faq.map((item) => (
          <details key={item.q} className="group py-3">
            <summary className="flex cursor-pointer list-none items-center justify-between gap-3 font-medium text-gray-800 transition hover:text-brand-700 [&::-webkit-details-marker]:hidden">
              {item.q}
