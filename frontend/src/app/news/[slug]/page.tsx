...
export default async function NewsDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
      <Link href="/news" className="inline-flex items-center gap-1 text-sm text-gray-500 hover:text-brand-600">
        <ChevronLeft size={16} />
        返回资讯列表
      </Link>
      <article className="mt-6 overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
        ...
        <div className="p-6 sm:p-10">
          ...
          <h1 className="mt-3 text-2xl font-bold leading-9 text-gray-900 sm:text-3xl">{item.title}</h1>
          <div
            className="prose prose-gray mt-6 max-w-none prose-a:text-brand-600"
        ...
        <nav className="mt-6 grid gap-3 sm:grid-cols-2" aria-label="上下篇导航">
          {newer ? (
            <Link
              ...
              className="group rounded-2xl border border-gray-200 bg-white p-4 transition hover:border-brand-300"
            >
              <span className="flex items-center gap-1 text-xs text-gray-400">
                <ChevronLeft size={13} />
                上一篇
              </span>
              <span className="mt-1 line-clamp-1 text-sm font-medium text-gray-800 group-hover:text-brand-700">
                {newer.title}
              </span>
            </Link>
          ) : (
          ...
          {older && (
            <Link
              ...
              className="group rounded-2xl border border-gray-200 bg-white p-4 text-right transition hover:border-brand-300"
            >
      ...
      {related.length > 0 && (
        <section className="mt-10">
          <h2 className="text-lg font-bold text-gray-900">相关资讯</h2>
          <div className="mt-4 space-y-3">
            {related.map((n) => (
              <Link
                key={n.cid}
                href={`/news/${n.slug}`}
                className="block rounded-2xl border border-gray-200 bg-white p-4 transition hover:border-brand-300 hover:shadow-sm"
              >
